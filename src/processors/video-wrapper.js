...
function buildWrapperOverlayFilters({
  ...
  console.log(`[video-wrapper.js] Wrapper geometry: ${sourceWidth}x${sourceHeight} → ${canvasW}x${canvasH}, border=${borderW}, top=${topBarH}, bottom=${bottomBarH}, cta=${footerCtaH}, side=${sidePanelW}`);
...
export async function brandVideoWrapper({
  ...
  if (filters.length > 0) {
    ...
    const args = [
      ...
      "-c:v", "libx264",
      "-preset", "fast",
      "-crf", "18",      // Higher quality for wrapper jobs
      "-c:a", "copy",
      "-shortest",
      outputPath,
    ];
